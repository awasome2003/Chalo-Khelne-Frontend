      ...
      {isLoading ? (
        ...
        <div className="space-y-2">
          {forums.map((forum) => (
            ...
            >
              <div className="w-12 h-12 rounded-xl bg-[#004E93]/10 flex items-center justify-center text-2xl flex-shrink-0">
                {forum.icon}
              </div>
            ...
            <div className="p-6 space-y-4">
                ...
                <div className="flex gap-2">
                  {["💬", "🏓", "🏸", "⚽", "🏏", "🎾", "🏆", "💪", "📢", "🔥"].map((emoji) => (
                    <button key={emoji} onClick={() => setNewIcon(emoji)}
                      className={`w-10 h-10 rounded-xl text-xl flex items-center justify-center transition w-auto ${newIcon === emoji ? "bg-[#004E93]/10 ring-2 ring-[#004E93]" : "bg-gray-100 hover:bg-gray-200"}`}>
                      {emoji}
                    </button>
                  ))}
                </div>
              ...
              <div className="flex justify-end gap-3 pt-2">
                ...
                <button onClick={handleCreate} disabled={!newName.trim() || createMutation.isPending}
                  className="px-5 py-2.5 bg-[#004E93] text-white text-sm font-bold rounded-xl transition disabled:opacity-50 flex items-center gap-2 w-auto">
